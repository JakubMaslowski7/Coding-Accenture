...
omega=1.1; %Relaxation
[T_gaussSeidel,nIter,residuals_gaussSeidel]=GaussSeidel2(T0,A,B,maxit,toll,omega);
Tteo=c1*exp(mu1*x0)+c2*exp(mu2*x0)-Sc/Sp;
plot(x0,T_gaussSeidel,'mv'); hold on;


omega=1.2; %Relaxation
[T_gaussSeidel,nIter,residuals_gaussSeidel]=GaussSeidel2(T0,A,B,maxit,toll,omega);
Tteo=c1*exp(mu1*x0)+c2*exp(mu2*x0)-Sc/Sp;
plot(x0,T_gaussSeidel,'mv');